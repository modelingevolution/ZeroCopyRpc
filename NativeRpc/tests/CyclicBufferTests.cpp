...
protected:
	void  SetUp() override
	{
        buffer = new CyclicBuffer( CAPACITY, BUFFER_SIZE);
    }
...
TEST_F(CyclicBufferTest, MultipleCursorsChasingIndependently) {
    ...
    auto cursor1 = buffer->OpenCursor();
    ...
    auto cursor2 = buffer->OpenCursor();
...
TEST_F(CyclicBufferTest, RemainingWithSpecificStartIndex) {
    ...
    auto cursor = buffer->OpenCursor(1);  // Start from index 1
